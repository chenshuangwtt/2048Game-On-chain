...
import { fetchTopPlayers } from "@/lib/subgraph";
import {
  formatTimestamp,
  formatUpdatedAt,
  shortenAddress,
...
export default function Leaderboard({
  ...
  const enabled = hasContract && Boolean(SUBGRAPH_URL);

  const query = useQuery({
    queryKey: leaderboardKey(SCORE_CONTRACT_ADDRESS, SUBGRAPH_URL),
    enabled,
  ...
  const errorMessage = !hasContract
    ...
    : query.error instanceof Error
      ? `Subgraph 排行榜加载失败：${query.error.message}`
      : query.error
        ? "Subgraph 排行榜加载失败，请稍后重试。"
        : null;
    ...
    return (
      <div className="flex flex-col gap-2">
        {entries.map((entry, index) => (
          <div
            key={`${entry.player}-${entry.lastPlayedAt}-${index}`}
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex flex-col items-end gap-1">
                ...
                <span className="text-[10px] text-[var(--primary-text-color)]">
                  {entry.lastPlayedAt ? formatTimestamp(entry.lastPlayedAt) : "-"}
                </span>
              </div>
            </div>
        ))}
